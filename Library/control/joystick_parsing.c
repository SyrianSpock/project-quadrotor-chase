...
/**
 * \brief						Switch on the mode custom flag
 *
 * \param	joystick_parsing	The pointer to the joystick parsing structure
 * \param	button				The value of the button pressed
 */
void joystick_parsing_button_custom_on(joystick_parsing_t* joystick_parsing, button_pressed_t button);

/**
 * \brief						Switch off the mode custom flag
 *
 * \param	joystick_parsing	The pointer to the joystick parsing structure
 * \param	button				The value of the button pressed
 */
void joystick_parsing_button_custom_off(joystick_parsing_t* joystick_parsing, button_pressed_t button);
...
void joystick_parsing_button_custom_on(joystick_parsing_t* joystick_parsing, button_pressed_t button)
{
	if (button == BUTTON_PRESSED)
	{
		joystick_parsing->state->mav_mode.byte &= 0b11111110;
		joystick_parsing->state->mav_mode.byte += 1;
	}
}
...
void joystick_parsing_button_mask(joystick_parsing_t* joystick_parsing, uint16_t buttons)
{
	...
	joystick_parsing_button_custom_on(joystick_parsing, button_local.button_7);
	joystick_parsing_button_custom_off(joystick_parsing, button_local.button_4);
